...
import { DEFAULT_COURSE_ID, DEFAULT_COURSE_PROGRESS } from "@/constants/default";
import {
...
export function CourseProvider({ children }: Props) {
  ...
  useEffect(() => {
    const initializeCourse = async () => {
      try {
        // Check if onboarding is completed (mobile only)
        ...
        // If onboarding not completed, leave courseId as null
        // This prevents auto-redirect to /learn before onboarding
        ...
        if (
          ...
          // Only set default course if onboarding is completed
          handleCourseProgress(DEFAULT_COURSE_ID);
          setCourseId(DEFAULT_COURSE_ID);
        }
      } catch (error) {
        console.error("Error fetching course ID:", error);
        // Handle the error gracefully - but still check onboarding
        if (Platform.OS !== "web") {
          const onboardingStatus = await AsyncStorage.getItem("onboarding_completed");
